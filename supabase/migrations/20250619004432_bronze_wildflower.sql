/*
  ...
  1. New Tables
    ...
    - `reports` - Store user-submitted wait time and capacity reports

  2. Views and Functions
    - `aggregated_wait` - Materialized view for calculating average wait times
    - `hospital_wait_times` - View combining hospitals with wait time data
    - `daily_report_counts` - Analytics view for daily reporting statistics
    - `nearby_hospitals()` - Function to find hospitals within a radius
    - `refresh_aggregated_wait()` - Function to refresh materialized view
  ...
  4. Performance
    ...
    - Materialized view for fast wait time calculations
*/
...
DROP MATERIALIZED VIEW IF EXISTS public.aggregated_wait;
